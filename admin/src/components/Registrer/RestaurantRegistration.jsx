...
const RestaurantRegistration = () => {
  ...
  const url = `${import.meta.env.VITE_API_URL || 'http://localhost:8080/api/v1'}/restaurant`
  const [data, setData] = useState({
    ...
    openingTime: "",
    closingTime: "",
    // Note: Avatar image binary is managed separately via image state
  });
  ...
  const onSubmitHandler = async (e) => {
    ...
    // Basic client-side check - taaki bina image ke request hi na jaaye
    if (!image) {
      toast.error("Please select a restaurant image before submitting.");
      return;
    }
    ...
    try {
      ...
      // Ab real backend error message dikhega (jaise "User not found",
      // "All fields are required", etc.) generic message ki jagah
